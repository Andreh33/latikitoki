...
const auroraFragment = /* glsl */ `
  ...
  void main(){
    ...
    // Cintas de aurora sobre noche profunda (umbrales altos = más oscuro)
    vec3 col = base;
    ...
    col = mix(col, rosa, smoothstep(0.62, 1.08, n * n2) * 0.7);

    col *= 0.9;
    float d = distance(uv, vec2(0.5));
    col *= smoothstep(1.25, 0.12, d);
    col += violeta * 0.05 * (1.0 - d);
...
function Crystal({ lite }: { lite: boolean }) {
  ...
  return (
    <Float speed={1.6} rotationIntensity={0.5} floatIntensity={1.1}>
      <mesh ref={mesh} scale={lite ? 1.7 : 1.95} rotation={[0.3, 0.4, 0]}>
        {/* gema facetada de baja poligonización */}
